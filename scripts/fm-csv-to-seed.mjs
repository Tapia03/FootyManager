...
const MIN_REP = 2000;   // FM 0-10000 — mais baixo agora que há ligas menores
const MAX_CLUBS_PER_LEAGUE = 20;
const MIN_CLUBS_PER_LEAGUE = 12;
const MIN_SQUAD = 11;
const VALUE_PLACEHOLDERS = new Set(["-1", "0", "300000000"]); // Genie Scout usa esses quando não sabe
...
function loadCsv(file) {
  const text = fs.readFileSync(path.join(SRC_DIR, file)).toString("latin1");
...
const num = (s) => parseFloat(digits(s).replace(",", ".")) || 0;
...
const POS_RATING_COLS = ["GR", "L", "D E", "D C", "D D", "A E", "A D", "MD", "M E", "M C", "M D", "MA E", "MA C", "MA D", "PLR", "PLA"];
...
function mapPositionFromRatings(vals, name = "") {
  let bi = 0;
...
function derivedValue(overall, age) {
...
function derivedWage(overall) {
  return Math.round(3000 + Math.pow(Math.max(overall - 40, 1), 2.1) * 22);
}

// --- Roda ---------------------------------------------------------------
const C = loadCsv("CLUBES.csv");
const P = loadCsv("JOGADORES.csv");
const cId = (n) => C.idx(n);
const pId = (n) => P.idx(n);
...
// filiais; um número redondo por liga mantém o calendário são. Ligas que não
// juntam o mínimo são descartadas por inteiro mais abaixo.
{
  const byDiv = new Map();
...
// A base do FM lista o elenco inteiro (base, reservas, emprestados) — clubes
// grandes vêm com 60-98 nomes. Corta pra um plantel de tamanho de jogo: os
// melhores por overall, garantindo um mínimo por setor pra não faltar goleiro.
// Liga jogável guarda 30; liga de segundo plano guarda 24 (XI + reservas +
// um pouco de banco), o bastante pra tela de elenco/olheiro sem inflar o banco.
...
for (const c of clubs.values()) {
  ...
  // Força média do elenco (top 18 por overall) — cache usado pela simulação
  // leve das ligas de segundo plano e pela virada de temporada rápida, pra o
  // motor nunca precisar carregar os jogadores desses clubes.
...
// Só clubes com elenco jogável de verdade (>= MIN_SQUAD e com goleiro). Esta
// base do FM ("DB COM DADOS 2026", editada com foco no Brasil) traz várias
// divisões estrangeiras com elencos truncados/vazios — este filtro tira o lixo.
let finalClubs = [...clubs.values()].filter(
  (c) => c.players.length >= MIN_SQUAD && c.players.some((p) => p.position === "GK"),
);

// Descarta ligas que, DEPOIS do filtro acima, não juntaram o mínimo de clubes
// (calendário quebra sem isso), e força número PAR de clubes por liga (round
// robin não lida bem com bye) — corta o(s) de menor reputação.
{
...
// Remove os campos internos (_divId/_playable) antes de serializar.
for (const c of finalClubs) { delete c._divId; delete c._playable; }
const seed = {
